...
let recent:[Hitter] = hallOfFame.filter({ (item: Hitter)-> Bool in
    
            return item.year > 2006

    })


//: sorted 함수를 이용해 최근부터 오래된 순으로 기록을 나열한 latest 배열을 만들어라.
let latest:[Hitter] = hallOfFame.sorted(by: {(left:Hitter, right:Hitter) -> Bool in
...
var totalAverage:Double = hallOfFame.reduce(0.0) { (Result:Double, hitter:Hitter) -> Double in
    
    return (Result + hitter.average)
}

totalAverage /= Double(hallOfFame.count)
